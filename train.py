...
def pad_to_multiple_of_8(x):
    ...
    pad_w = (8 - w % 8) % 8
    if h + pad_h < min_size:
        pad_h += min_size - (h + pad_h)
    if w + pad_w < min_size:
        pad_w += min_size - (w + pad_w)
...
def warp(frame, flow):
    ...
    grid = torch.stack((grid_x, grid_y), dim=0).float().to(frame.device)
    grid = grid.unsqueeze(0).repeat(B, 1, 1, 1)
    vgrid = grid + flow
    vgrid = vgrid.permute(0, 2, 3, 1)
    vgrid[..., 0] = 2.0 * vgrid[..., 0] / (W - 1) - 1.0
    vgrid[..., 1] = 2.0 * vgrid[..., 1] / (H - 1) - 1.0
    return F.grid_sample(frame, vgrid, mode='bilinear', padding_mode='border', align_corners=True)
...
def training(gs_type, dataset: ModelParams, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint,
    ...
    raft_model = raft_large(weights=Raft_Large_Weights.C_T_V2, progress=False).to('cuda')
    raft_model.eval()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        #         gt_prev = outputs["camera_prev"]["gt"]
        #
        #         prev = gt_prev.detach().unsqueeze(0)
        #         curr = gt_curr.detach().unsqueeze(0)
        #
        #         prev_padded, h, w = pad_to_multiple_of_8(prev)
        #         curr_padded, _, _ = pad_to_multiple_of_8(curr)
        #         with torch.no_grad():
        #             flows = raft_model(prev_padded, curr_padded)
        #         flow_prev_to_curr = flows[-1][:, :, :h, :w]
        #         gt_prev_warped = warp(prev, flow_prev_to_curr).squeeze(0)
        #         L_flow_temporal += l1_loss(render_curr, gt_prev_warped)
        #
        #     if camera_next is not None:
        #         gt_next = outputs["camera_next"]["gt"]
        #
        #         curr = gt_curr.detach().unsqueeze(0)
        #         next = gt_next.detach().unsqueeze(0)
        #
        #         curr_padded, h, w = pad_to_multiple_of_8(curr)
        #         next_padded, _, _ = pad_to_multiple_of_8(next)
        #         with torch.no_grad():
        #             flows = raft_model(curr_padded, next_padded)
        #         flow_curr_to_next = flows[-1][:, :, :h, :w]
        #         gt_next_warped = warp(next, -flow_curr_to_next).squeeze(0)
        #         L_flow_temporal += l1_loss(render_curr, gt_next_warped)


        #print("AAA", Ll1.item(), Ll1_inter.item(), ssim_loss)
        loss = 2.0 * Ll1 + 0.5 * Ll1_inter +  0.25 * sigma_loss + 1 * ssim_loss#+ 0.1 * L_flow_temporal #+ 0.1 * sigma_loss ##
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[5_000, 10_000, 15_000, 20_000, 25_000, 30_000,100_000])
